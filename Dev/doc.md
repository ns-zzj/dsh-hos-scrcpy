...
> 注意：**DSH 插件本体（Host / Client 半区）不在这里**，它们在 `PluginMain-Dynamic/`（动态版）和 `PluginMain-Static/`（静态版）。
...
```
Dev/src/Main.java  ──javac 编译──▶  PluginMain-Dynamic/out/            （动态版运行时用）
                                  PluginMain-Static/resources/out/     （静态版运行时用）
Dev/demo/jmuxer.min.js ──复制──▶  PluginMain-Dynamic/jmuxer.min.js
                                  PluginMain-Static/resources/jmuxer.min.js
                                  （三处副本 MD5 一致，改一处必须同步另外两处）
```

- 根目录 `hosScrcpy-1.0.18-beta.jar` 不存在，SDK jar 以副本形式分别存在于
  `PluginMain-Dynamic/` 与 `PluginMain-Static/resources/`（编译期和运行期都要用，无需从 Dev 复制）。
- 根目录 `dsh-hos-scrcpy-1.0.0.tgz` 是 `PluginMain-Static` 的 `npm pack` 产物。
...
# 静态版产物（覆盖 PluginMain-Static/resources/out/，随后需重新 npm pack）
...
> ⚠️ 两处 `out/` 是**各自独立**的编译产物（class 字节不同，即使源码相同），改源码后**必须分别重新编译**。
> 静态版重新编译后要 `cd PluginMain-Static && npm pack` 重新打包再装。
...
- 三处副本同源（`Dev/demo/`、`PluginMain-Dynamic/`、`PluginMain-Static/resources/`，MD5 一致），
  升级库时三处一起替换。
...
| 要改什么 | 位置 | 流程 |
|---|---|---|
| sidecar 行为（协议/命令/流控制） | `Dev/src/Main.java` | 改源码 → 编译同步到两个 `out/` → 用 demo 页独立验证 → 再在 DSH 里验证；静态版还要重新 `npm pack` |
| 协议格式 | `Main.java` + `demo/index.html` + 插件 `client.js` | 三处协议必须同步改（sidecar 收发、测试页收发、插件 Client 收发） |
| 前端交互/UI | 插件 `PluginMain-*/client.js` | 只改插件，不用动 Dev |
...
- `PluginMain-Static/README.md`：静态版包结构、安装步骤、与动态版差异对照表
