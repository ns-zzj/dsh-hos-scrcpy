...
let shotToolRegisterError = ''   // 最近一次工具注册失败的原始错误（透传给前端排障）
...
const handlers = {
  ...
  'shot:set-enabled': async function (args, ctx) {
    ...
    syncShotTool(ctx)
    return { ok: true, enabled: enabled, registered: !!shotToolDisposer, count: Object.keys(shotEnabledSns).length, registerError: shotToolRegisterError }
  },
