// ============================================================
// dsh-hos-scrcpy — 插件 Client 半区（静态 npm 包浏览器 bundle）
// 由 PluginMain-Dynamic/client.js 改造生成：
//   host.call -> rpc()（同源 fetch POST /dsh-hos-scrcpy/rpc）
//   styles.insert -> insertStyles()（<style> 注入）
//   ctx.timer.interval -> window.setInterval
//   inject: ['timer'] -> exports.inject = ['slots']
// 格式遵循 dsh-client-modules 的 window.__ModuleLoader__.load 约定。
// ============================================================
...
function rpc(method, args) {
  return fetch('/dsh-hos-scrcpy/rpc', {
    ...
    if (!res || res.ok !== true) throw new Error((res && res.error) || 'RPC 失败: ' + method)
    return res.result
  })
}
