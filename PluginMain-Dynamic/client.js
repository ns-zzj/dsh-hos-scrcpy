...
.dshhos-item {
  ...
  transition: background .12s ease; cursor: pointer; border: 0; background: transparent; width: 100%; text-align: left; box-sizing: border-box;
}
.dshhos-item:hover { background: var(--dsw-alias-interactive-bg-hover, rgba(38,49,72,.06)); }
.dshhos-item:disabled { opacity: .6; cursor: default; }
.dshhos-item .dshhos-connect { margin-left: auto; font-size: 11px; color: var(--dsw-alias-state-business-primary, #4176e6); font-family: inherit; }
...
return {
  ...
  apply(ctx) {
    ...
    function ControlPanel(props) {
      ...
      React.useEffect(function () {
        ...
        async function init() {
          try {
            ...
            if (!JMuxerCtor) {
              const src = await host.call('jmuxer:source', {})
            ...
            timerDispose = ctx.timer.interval(function () {
              if (disposed) return
              const now = Date.now()
      ...
      function disconnect() {
        try { if (wsRef.current) wsRef.current.close() } catch (e) {}
        host.call('device:disconnect', {}).catch(function () {})
        onClose()
      }
      ...
      return React.createElement('div', { className: 'dshhos-control', style: { width: panelW + 'px' } },
        React.createElement('div', { className: 'dshhos-control-head', ref: headRef },
          React.createElement('span', { className: 'dshhos-csn' }, sn),
    ...
    function DevicePanel() {
      ...
      async function connectDevice(sn) {
        ...
        setError('')
        try {
          const r = await host.call('device:connect', { sn: sn })
          if (r && r.ok && r.port) {
            setOpen(false)
      ...
      async function saveConfig() {
        ...
        setNotice('')
        try {
          const r = await host.call('cfg:save', { javaPath: draft.javaPath, hdcPath: draft.hdcPath })
      ...
      return React.createElement('div', { className: 'dshhos-trigger' },
        ...
        control ? React.createElement(ControlPanel, { sn: control.sn, port: control.port, onClose: function () { setControl(null) } }) : null,
                  ...
                  ? React.createElement('div', { className: 'dshhos-list' },
                      devices.map(function (dev) {
                        return React.createElement('button', {
                          className: 'dshhos-item', key: dev.sn, disabled: !!connectingSn,
                          onClick: function () { connectDevice(dev.sn) },
                        },
                          React.createElement('span', { className: 'dshhos-dot ok' }),
                          React.createElement('span', null, dev.sn),
                          connectingSn === dev.sn
                            ? React.createElement('span', { className: 'dshhos-connecting' }, '连接中…')
                            : React.createElement('span', { className: 'dshhos-connect' }, '投屏'),
                        )
                      }),
                    )
    ...
    slots.inject('conversation.session.header.utilities', function () {
      return slots.register(
        ...
        function () { return React.createElement(DevicePanel) },
      )
    })
  },
}
